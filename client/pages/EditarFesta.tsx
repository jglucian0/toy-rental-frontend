...
export default function EditarFesta() {
  ...
  const [brinquedosOriginaisDaFesta, setBrinquedosOriginaisDaFesta] = useState([]);
    ...
    fetch(`http://localhost:8000/api/festas/${id}/`)
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error("Erro ao carregar festa");
        }
        const data = await res.json();
        console.log("Festa carregada:", data);
        setFesta(prev => ({
          ...prev,
          ...data,
          brinquedos_ids: data.brinquedos.map(b => Number(b.id)),
          cliente_id: data.cliente?.id || "",
          nome: data.cliente?.nome || "",
          documento: data.cliente?.documento || "",
          telefone: data.cliente?.telefone || "",
          cep: data.cliente?.cep || prev.cep,
          rua: data.cliente?.rua || prev.rua,
          numero: data.cliente?.numero || prev.numero,
          bairro: data.cliente?.bairro || prev.bairro,
          cidade: data.cliente?.cidade || prev.cidade,
          uf: data.cliente?.estado || prev.uf,       // observação abaixo sobre uf/estado
          pais: data.cliente?.pais || prev.pais,
          complemento: data.cliente?.complemento || prev.complemento,
        }));
        setBrinquedosOriginaisDaFesta(data.brinquedos || []);
        setBrinquedosSelecionados(data.brinquedos || []);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    const entradaSugerida = Number(festa.valor_total) * 0.3;
    setFesta(prev => ({
      ...prev,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {brinquedosDisponiveis.map((brinquedo) => (
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={festa.brinquedos_ids.includes(Number(brinquedo.id))}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setBrinquedosSelecionados(prev => [...prev, brinquedo]);
                                  setFesta(prev => ({
                                    ...prev,
                                    brinquedos_ids: [...prev.brinquedos_ids, Number(brinquedo.id)],
                                  }));
                                } else {
